...
require_relative '../models/money_movement.rb'
...
class Rental
  ...
  class << self
    ...
    def to_money_movements_hash
      { rentals: all.map { |rental| rental.money_movements_hash } }
    end
  end
  ...
  def rental_days
    @rental_days ||=
      duration.times.with_object([]) do |day_number, rental_days|
  ...
  def money_movements
    @money_movements ||=
      MoneyMovement::TYPE.each_with_object([]) do |type, money_movements|
        money_movements << MoneyMovement.new(type, self)
      end
  end

  def options
    @options ||= Option.find_all_by_rental(self)
  end
  ...
  def price_without_options
    @price_without_options ||= time_price + distance_price
  end

  def price
    @price ||= time_price + distance_price + options_price
  end
  ...
  def price_hash
    {
      ...
      commission:
        {
          "insurance_fee": MoneyMovement.new("insurance", self).amount,
          "assistance_fee": MoneyMovement.new("assistance", self).amount,
          "drivy_fee": MoneyMovement.new("drivy", self).amount
        }
    }
  end

  def money_movements_hash
    {
      id: id,
      options: options.map(&:type).uniq,
      actions: money_movements.map(&:to_hash)
    }
  end
